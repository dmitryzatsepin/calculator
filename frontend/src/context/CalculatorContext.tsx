...
import type { // Только те, что ДЕЙСТВИТЕЛЬНО нужны для приведения типов или уникальных структур
  Location as GqlLocation, // Используется в useEffect для selectedProtectionCode
  Maybe,
...
export const CalculatorProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    if (rawModuleDetails) {
      ...
      const mappedModuleForState: ModuleData = {
        ...
        components: rawModuleDetails.items?.map(comp => ({
          quantity: comp?.quantity ?? 0, itemCode: comp?.item?.code ?? "unknown",
          itemName: comp?.item?.name ?? "Unknown Item", itemSku: comp?.item?.sku,
        })).filter(c => c.itemCode !== "unknown") ?? [],
      };
      if (JSON.stringify(mappedModuleForState) !== JSON.stringify(selectedModuleDetailsState)) {
        setSelectedModuleDetailsState(mappedModuleForState);
      }
    } else if (selectedModuleDetailsState !== null) {
  ...
  useEffect(() => {
    if (rawCabinetDetails) {
      ...
      if (JSON.stringify(mappedCabinetForState) !== JSON.stringify(selectedCabinetDetailsState)) {
        setSelectedCabinetDetailsState(mappedCabinetForState);
      }
    } else if (selectedCabinetDetailsState !== null) {
  ...
  useEffect(() => {
    ...
    const selectedLocationItem = locations.find((loc): loc is GqlLocation => loc?.code === selectedLocationCode); // Используем 'locations'
  ...
  useEffect(() => {
    if (!isLoadingRefreshRates && gqlFilteredRefreshRates.length > 0 && selectedRefreshRateCode === null) {
      ...
      setSelectedRefreshRateCodeState(defaultRefreshRate?.code ?? null); // Добавил ?? null
    ...
    if (!isLoadingBrightnesses && gqlFilteredBrightnesses.length > 0) {
      ...
      if (!currentSelectionValid || selectedBrightnessCode === null) {
        ...
        if (defaultBrightness?.code && defaultBrightness.code !== selectedBrightnessCode) {
            setSelectedBrightnessCodeState(defaultBrightness.code);
  ...
  const screenTypeSegments = useMemo((): SelectOption[] => screenTypes.map(st => ({ value: st.code ?? "", label: st.name ?? st.code ?? "" })), [screenTypes]);
  const locationSelectOptions = useMemo((): SelectOption[] => locations.filter(loc => loc.active).map(loc => ({ value: loc.code ?? "", label: loc.name ?? loc.code ?? "" })), [locations]);
  const materialSelectOptions = useMemo((): SelectOption[] => materials.filter(mat => mat.active).map(mat => ({ value: mat.code ?? "", label: mat.name ?? mat.code ?? "" })), [materials]);
  ...
  const pitchSelectOptions = useMemo((): SelectOption[] => [...gqlFilteredPitches].filter(p => !!p.code).sort((a,b) => (a.pitchValue ?? 0) - (b.pitchValue ?? 0)).map(p => ({ value: p.code ?? "", label: `${p.pitchValue} мм` })), [gqlFilteredPitches]);
  const brightnessSelectOptions = useMemo((): SelectOption[] => gqlFilteredBrightnesses.filter(br => !!br.code).map(br => ({ value: br.code ?? "", label: `${br.value} nit` })), [gqlFilteredBrightnesses]);
  const refreshRateSelectOptions = useMemo((): SelectOption[] => gqlFilteredRefreshRates.filter(rr => !!rr.code).map(rr => ({ value: rr.code ?? "", label: `${rr.value} Hz` })), [gqlFilteredRefreshRates]);
  const sensorSelectOptions = useMemo((): SelectOption[] => sensors.filter(s => s.active && !!s.name).sort((a,b) => (a.name ?? "").localeCompare(b.name ?? "")).map(s => ({ value: s.code ?? "", label: s.name ?? "" })), [sensors]);
  const controlTypeSelectOptions = useMemo((): SelectOption[] => controlTypes.filter(ct => ct.active && !!ct.name).sort((a,b) => (a.name ?? "").localeCompare(b.name ?? "")).map(ct => ({ value: ct.code ?? "", label: ct.name ?? "" })), [controlTypes]);
  ...
  const isCalculationReady = useMemo((): boolean => {
    const widthVal = Number(widthMm); const heightVal = Number(heightMm); const rateVal = Number(localDollarRateInput);
    const baseChecks = !!selectedScreenTypeCode && widthVal > 0 && heightVal > 0 && !!selectedLocationCode &&
                       !!selectedProtectionCode && !!selectedPitchCode && !!selectedBrightnessCode && !!selectedRefreshRateCode &&
                       !!selectedModuleCode && !!selectedModuleDetailsState && !isLoadingModuleDetails && !isErrorModuleDetails &&
                       rateVal > 0 && !isCalculating && !isLoadingPrices && !isErrorPrices && Object.keys(priceMap).length > 0;
    if (!baseChecks) return false;
    if (isCabinetScreenTypeSelected) {
      return !!selectedMaterialCode && !!selectedCabinetCode && !!selectedCabinetDetailsState && !isLoadingCabinetDetails && !isErrorCabinetDetails;
    }
    return true;
  }, [
  ...
  // --- Функции-сеттеры (useCallback) ---
  const setSelectedScreenTypeCode = useCallback((value: string | null) => { setSelectedScreenTypeCodeState(value); setSelectedLocationCodeState(null); setSelectedMaterialCodeState(null); setSelectedProtectionCodeState(null); setSelectedBrightnessCodeState(null); setSelectedRefreshRateCodeState(null); setSelectedSensorCodesState([]); setSelectedControlTypeCodesState([]); setSelectedPitchCodeState(null); setSelectedModuleCodeState(null); setSelectedCabinetCodeState(null); setIsFlexSelectedState(false); setCalculationResultState(null); setCostDetailsState(null); }, []);
  const setSelectedLocationCode = useCallback((value: string | null) => { setSelectedLocationCodeState(value); setSelectedMaterialCodeState(null); setSelectedBrightnessCodeState(null); setSelectedRefreshRateCodeState(null); setSelectedPitchCodeState(null); setSelectedModuleCodeState(null); setSelectedCabinetCodeState(null); setCalculationResultState(null); setCostDetailsState(null); }, []);
  const setSelectedMaterialCode = useCallback((value: string | null) => { setSelectedMaterialCodeState(value); setSelectedCabinetCodeState(null); setCalculationResultState(null); setCostDetailsState(null); }, []);
  const setSelectedProtectionCode = useCallback((value: string | null) => setSelectedProtectionCodeState(value), []);
  const setSelectedBrightnessCode = useCallback((value: string | null) => { setSelectedBrightnessCodeState(value); setSelectedModuleCodeState(null); setSelectedCabinetCodeState(null); setCalculationResultState(null); setCostDetailsState(null); }, []);
  const setSelectedRefreshRateCode = useCallback((value: string | null) => { setSelectedRefreshRateCodeState(value); setSelectedBrightnessCodeState(null); setSelectedModuleCodeState(null); setSelectedCabinetCodeState(null); setCalculationResultState(null); setCostDetailsState(null); }, []);
  ...
  const setSelectedPitchCode = useCallback((value: string | null) => { setSelectedPitchCodeState(value); setSelectedRefreshRateCodeState(null); setSelectedBrightnessCodeState(null); setSelectedModuleCodeState(null); setSelectedCabinetCodeState(null); setCalculationResultState(null); setCostDetailsState(null); }, []);
  const setSelectedModuleCode = useCallback((value: string | null) => { setSelectedModuleCodeState(value); setSelectedCabinetCodeState(null); setCalculationResultState(null); setCostDetailsState(null); }, []);
  const setSelectedCabinetCode = useCallback((value: string | null) => { setSelectedCabinetCodeState(value); setCalculationResultState(null); setCostDetailsState(null); }, []);
  const setIsFlexSelected = useCallback((selected: boolean) => { setIsFlexSelectedState(selected); setCalculationResultState(null); setCostDetailsState(null);}, []);
  const setLocalDollarRateInput = useCallback((value: number | string) => setLocalDollarRateInputState(value), []);
  const setWidthMm = useCallback((value: string | number) => { setWidthMmState(value); setCalculationResultState(null); setCostDetailsState(null); }, []);
  const setHeightMm = useCallback((value: string | number) => { setHeightMmState(value); setCalculationResultState(null); setCostDetailsState(null); }, []);

  const performCalculation = useCallback(async () => {
    if (!isCalculationReady || !selectedModuleDetailsState || (isCabinetScreenTypeSelected && !selectedCabinetDetailsState) || !priceMap) {
      console.warn("Calculation prerequisites not met or essential data missing (performCalculation). Details:", {isCalculationReady, selectedModuleDetailsState, isCabinetScreenTypeSelected, selectedCabinetDetailsState, priceMap});
      return;
    }
    setIsCalculatingState(true); setCalculationResultState(null); setCostDetailsState(null);
    try {
      const pitchObject = gqlFilteredPitches.find(p => p?.code === selectedPitchCode);
      const pitchValue = pitchObject?.pitchValue;
      if (typeof pitchValue !== 'number') throw new Error(`Pitch value for code '${selectedPitchCode}' not found or not a number.`);
      ...
      const formData: CalculatorFormData = {
        ...
        selectedScreenWidth: Number(widthMm) || 0, selectedScreenHeight: Number(heightMm) || 0,
        selectedScreenTypeCode: selectedScreenTypeCode ?? "",
        moduleItemComponents: selectedModuleDetailsState.components?.map(c => ({ itemCode: c.itemCode, itemName: c.itemName, countPerModule: c.quantity })) ?? [],
      };
      const techSpecResult = calculateTechnicalSpecs(formData, selectedModuleDetailsState, isCabinetScreenTypeSelected ? selectedCabinetDetailsState : null);
      if (!techSpecResult) throw new Error("Technical spec calculation failed.");
      setCalculationResultState(techSpecResult);
      const costResult = calculateCosts(techSpecResult, priceMap, Number(localDollarRateInput), isCabinetScreenTypeSelected ? selectedMaterialCode : null);
      setCostDetailsState(costResult);
      setIsDrawerOpenState(true);
    } catch (calcError: any) {
      console.error("❌ Calculation failed:", calcError?.message ?? calcError);
      setCalculationResultState(null); setCostDetailsState(null);
    } finally {
    ...
    widthMm, heightMm, selectedScreenTypeCode, localDollarRateInput, calculateTechnicalSpecs, calculateCosts
  ]);

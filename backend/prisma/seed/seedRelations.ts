...
// Импортируем тип режима из index.ts (убедись, что он там экспортирован)
...
export async function seedRelations(
    ...
    idMaps: IdMaps, // Только читаем карты
    mode: SeedMode
): Promise<void> {
    console.log('\n--- Начало заполнения связующих таблиц ---');
    const startTime = Date.now();
    const allPromises: Promise<any>[] = []; // Для параллельного выполнения

    // --- Обработка простых связей M-N (таблицы без доп. полей) ---
    // Логика createMany({ skipDuplicates: true }) подходит для обоих режимов
    ...
    for (const config of relationMNConfigs) {
        ...
        if (!model || typeof model.createMany !== 'function') { // Проверяем и наличие createMany
        ...
        const mapRefKey1 = config.field1.mapRef.charAt(0).toLowerCase() + config.field1.mapRef.slice(1);
        const mapRefKey2 = config.field2.mapRef.charAt(0).toLowerCase() + config.field2.mapRef.slice(1);
        ...
        const dataToCreateMN: any[] = []; // Данные для createMany
        let skippedCountMN = 0;
        const processedPairsMN = new Set<string>(); // Для дубликатов в Excel (lowercase)

        for (const row of sheetData) {
            // Получаем ключи из Excel (оригинальный регистр)
            ...
            if (!key1Raw || !key2Raw) {
                // Можно добавить лог для отладки строк с частично пустыми ключами, если нужно
                // if(key1Raw || key2Raw) {
                //    console.warn(`[${config.modelName} Skip Row M-N] Отсутствует один из ключей: ${key1Raw}, ${key2Raw}.`, row);
                // }
                continue;
            }
            ...
            if (processedPairsMN.has(compositeKey)) {
                 // console.warn(`[${config.modelName} Duplicate Excel M-N] Пропуск дубликата: ${key1Raw} - ${key2Raw}`);
                 continue;
            }
            ...
            if (key1Exists && key2Exists) {
                // Добавляем в массив для createMany, используя ОРИГИНАЛЬНЫЕ КЛЮЧИ
                dataToCreateMN.push({
                ...
                skippedCountMN++;
                // Детальный лог для пропущенных из-за отсутствия ключей
                // console.warn(`[${config.modelName} Skip Relation M-N] Не найдены ключи в картах: ` +
                //              `${config.field1.excelCol}='${key1Raw}' (ищем '${lookupKey1Lower}' в '${mapRefKey1}', ${key1Exists ? 'найден' : 'НЕ найден'}), ` +
                //              `${config.field2.excelCol}='${key2Raw}' (ищем '${lookupKey2Lower}' в '${mapRefKey2}', ${key2Exists ? 'найден' : 'НЕ найден'}).`);
            }
        } // --- конец цикла по строкам для M-N ---
        ...
        if (dataToCreateMN.length > 0) {
             allPromises.push(
                // Используем переменную model, где уже есть (as any)
                model.createMany({
                    data: dataToCreateMN,
    ...
    for (const config of relationCountConfigs) {
        ...
        for (const row of sheetData) {
            ...
            if (entityExists && itemExists) {
                ...
                skippedCountCount++;
                // Детальный лог для пропущенных
                // console.warn(`[${config.modelName} Skip Relation Count] Не найдены ключи в картах: ` +
                //              `${config.entityField.excelCol}='${entityCodeRaw}' (ищем '${entityKeyLower}' в '${entityMapKey}', ${entityExists ? 'найден' : 'НЕ найден'}), ` +
                //              `${config.itemField.excelCol}='${itemCodeRaw}' (ищем '${itemKeyLower}' в '${itemMapKey}', ${itemExists ? 'найден' : 'НЕ найден'}).`);
            }
        } // --- конец цикла по строкам для связей с количеством ---
